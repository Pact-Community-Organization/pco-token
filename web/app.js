(() => {
  ...
  var CFG = {
    ...
    ns: "user",
    // devnet: the v2 (rounds) stack lives in `user`; `free` holds the retired v1
    chain: "0"
  ...
  var T = `${CFG.ns}.pco`;
  var C = `${CFG.ns}.pco-claim`;
  var G = `${CFG.ns}.pco-gas-station`;
  var API = `${CFG.host}/chainweb/0.0/${CFG.networkId}/chain/${CFG.chain}/pact/api/v1`;
  var b64url = (bytes) => btoa(String.fromCharCode(...bytes)).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");
  function hashCmd(cmd) {
    return b64url(blake2b2(new TextEncoder().encode(cmd), { dkLen: 32 }));
  }
  ...
  async function local(code) {
    const cmd = JSON.stringify({
      ...
      signers: [],
      meta: { chainId: CFG.chain, sender: "reader", gasLimit: 15e4, gasPrice: 1e-8, ttl: 600, creationTime: Math.floor(Date.now() / 1e3) - 30 },
  ...
  var num = (v) => v && typeof v === "object" ? Number(v.decimal ?? v.int ?? NaN) : Number(v);
  async function sponsored(code, extraCaps, dataObj) {
    ...
    const cmd = JSON.stringify({
      ...
      signers: [{
        ...
        clist: [
          ...extraCaps,
          { name: `${G}.GAS_PAYER`, args: ["web", { int: 6e3 }, { decimal: "0.0000001" }] }
        ]
      }],
      meta: { chainId: CFG.chain, sender: station, gasLimit: 6e3, gasPrice: 1e-8, ttl: 1800, creationTime: Math.floor(Date.now() / 1e3) - 30 },
  ...
  var state = { station: null, rounds: [], lastAction: 0, wallet: null };
  ...
  var ptime = (v) => new Date(v && typeof v === "object" ? v.time ?? v.timep : v);
  async function fetchOpenRounds() {
    ...
    for (const id of ids) {
      ...
      if (r.active && now >= ptime(r.opens) && now < ptime(r.closes) && num(r.claimed) + num(r.amount) <= num(r.budget)) {
  ...
  async function refresh() {
    ...
    try {
      ...
      $("claim-state").textContent = !open ? "CLOSED" : state.rounds.length ? `${state.rounds.length} open round${state.rounds.length > 1 ? "s" : ""}` : "no open rounds";
      ...
      renderWallet();
      if (already && Date.now() - state.lastAction > 8e3)
  ...
  async function claim() {
    ...
    if (!round) {
      setStatus("No open round to claim right now.", "err");
      return;
    }
    if (!code) {
      setStatus("Enter the engagement code first.", "err");
      return;
    }
  ...
  var AMOUNT_RE = /^\d+(\.\d{1,12})?$/;
  async function transfer(direction) {
    const w = state.wallet;
    if (!w) {
      setStatus("Connect a wallet to send PCO (transfers are not gas-sponsored).", "err");
      return;
    }
    const to = $("to").value.trim();
    const raw = $("amount").value.trim();
    if (!/^k:[0-9a-f]{64}$/.test(to)) {
      setStatus("Recipient must be a k: account (k: + 64 hex characters).", "err");
      return;
    }
    if (!AMOUNT_RE.test(raw) || Number(raw) <= 0) {
      setStatus("Amount must be a positive number (max 12 decimals).", "err");
      return;
    }
    ...
    if (direction === "x") {
      if (!/^([0-9]|1[0-9])$/.test(targetChain) || targetChain === CFG.chain) {
        setStatus("Cross-chain: pick a target chain 0\u201319 that is not the current chain.", "err");
        return;
      }
    } else if (to === w.account) {
      setStatus("Same-chain transfer to your own account is a no-op.", "err");
      return;
    }
    $("transfer-btn").disabled = true;
    $("xtransfer-btn").disabled = true;
  ...
  $("net").textContent = `${CFG.networkId} \xB7 chain ${CFG.chain} \xB7 ${CFG.ns}`;
  renderWallet();
  refresh();
